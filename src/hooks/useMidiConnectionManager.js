...
import { saveMidiDevicePreference, getLastMidiDevice, getMidiConnectionHistory, saveMidiConnectionHistory } from '../utils/midiStorageManager';
...
const useMidiConnectionManager = ({ 
   ...
   */
  const handleDeviceConnected = useCallback(e => {
   ...
   */
  const handleDeviceDisconnected = useCallback(e => {
   ...
   */
  const setupMidiHandlers = useCallback(input => {
    if (!input) return;
   ...
   * @param {Object} input - MIDI input device
   */
  const removeMidiHandlers = useCallback(input => {
    if (!input) return;
...
const connectToDevice = useCallback(deviceId => {
  ...
  try {
    ...
    if (autoRetry && retryAttemptsRef.current < maxRetryAttempts) {
      ...
      logDebug(`Will retry connection in ${delay / 1000} seconds. Attempt ${retryAttemptsRef.current + 1} of ${maxRetryAttempts}`);
