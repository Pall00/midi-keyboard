  ...
  switch (code) {
    case 'PERMISSION_DENIED':
      return {
        ...
        message:
          'You need to grant permission to access MIDI devices. Check your browser settings and try again.',
    ...
    case 'NOT_SUPPORTED':
      return {
        ...
        message:
          "Your browser doesn't support the Web MIDI API. Please use Chrome, Edge, or Opera.",
    ...
    case 'CONNECTION_FAILED':
      return {
        ...
        message:
          'Failed to connect to the MIDI device. Try disconnecting and reconnecting the device.',
      };
    default:
      return {
 ...
 */
const MidiConnectionStatus = ({ connectionStatus, selectedDevice, error, onRetry, className }) => {
  // Get status message
  const statusMessage = getStatusMessage(connectionStatus, selectedDevice?.name);

  // Error details
  const errorDetails = error ? getErrorDetails(error) : null;

  return (
    <StatusContainer $status={connectionStatus} className={className}>
      ...
      <div>
        ...
        {connectionStatus === CONNECTION_STATUS.ERROR && error && (
          <ErrorContainer>
            ...
            <ErrorActions>
              {onRetry && <RetryButton onClick={onRetry}>Retry Connection</RetryButton>}

              {errorDetails.helpUrl && (
                <HelpLink href={errorDetails.helpUrl} target="_blank" rel="noopener noreferrer">
                  Learn More
                </HelpLink>
              )}
            </ErrorActions>
